"""
Firm Dashboard — partner-level view of the entire firm.
All matters, all AI decisions, firm-wide metrics.
"""
...
st.set_page_config(page_title="Firm Dashboard", layout="wide", page_icon="🏛️")
...
st.title("🏛️ Firm Dashboard")
st.caption("Partner-level view — every matter, every AI decision across the entire firm.")
...
try:
    matters = requests.get(f"{API}/matters", timeout=8).json()
...
for col, val, lbl, color in [
    (cols[0], len(matters),  "Total Matters", "#0f172a"),
...
with col_list:
    st.markdown("**All Firm Matters**")
    sf = st.selectbox("Filter by status", ["All", "flagged", "assigned", "accepted", "completed"], label_visibility="collapsed")
    filtered = matters if sf == "All" else [m for m in matters if m["status"] == sf]
    selected_id = st.session_state.get("firm_selected", filtered[0]["id"] if filtered else None)
    ...
    for m in filtered:
        ...
        if st.button(label, key=f"firm_{m['id']}", use_container_width=True, type="primary" if is_sel else "secondary"):
            st.session_state["firm_selected"] = m["id"]
            st.rerun()
...
with col_detail:
    ...
    with tab_audit:
        ...
        for action in show:
            ...
            st.markdown(f"{badge} `{action['timestamp'][:16]}` **{action['role'].replace('_',' ').title()}** — {action['detail'][:80]}{'…' if len(action['detail'])>80 else ''}", unsafe_allow_html=True)

    with tab_doc:
        draft = selected.get("draft")
        if draft:
            st.text_area("Draft", value=draft[:1200] + ("…" if len(draft) > 1200 else ""), height=300, disabled=True)
            if len(draft) > 1200:
